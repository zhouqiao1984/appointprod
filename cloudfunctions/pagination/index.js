...
exports.main = async (event, context) => {
  ...
  let field = event.field ? event.field : null 
  ...
  return db.collection(dbName).where(filter).field(field).orderBy('_id', 'desc').skip((pageIndex - 1) * pageSize).limit(pageSize).get().then(res =>{
    res.hasMore = hasMore
    res.total = total // 总数
